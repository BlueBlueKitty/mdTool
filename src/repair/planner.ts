...
const formulaRules = new Set(["formula-trim", "formula-double-escape", "formula-code-wrapper"]);
...
export function createRepairPlan(text: string, revision: number, config: RuleConfig, options: PlanOptions): RepairPlan {
  ...
  for (const item of formulaRuleEdits(text, config.separatorMinLength, config.latexCommands)) { const itemCandidate = candidate(item.id, item.ruleId, item.nodeFrom, item.nodeTo, [item.edit], text); if (itemCandidate) raw.push(itemCandidate); }
  ...
  const filtered = raw.filter(item => {
    ...
    if (target.kind === "automatic" && item.confidence < options.settings.minimumConfidence) return false;
