...
const delimiterStyle = (enabled: Set<string>) => enabled.has("formula-delimiter-dollar") ? "dollar" : "latex";
function formulaEdits(result: FormulaRuleEdit[], text: string, nodeFrom: number, nodeTo: number, body: string, bodyFrom: number, kind: "inline" | "block", enabled: Set<string>, separatorMinLength: number, commands: string[]) {
  ...
  if (enabled.has("formula-trim")) { const leading = /^\s+/.exec(body); const trailing = /\s+$/.exec(body); if (leading) add(result, kind === "inline" ? "formula-trim" : "formula-block-normalize", nodeFrom, nodeTo, { from: bodyFrom, to: bodyFrom + leading[0].length, insert: "" }); if (trailing) add(result, kind === "inline" ? "formula-trim" : "formula-block-normalize", nodeFrom, nodeTo, { from: bodyFrom + body.length - trailing[0].length, to: bodyFrom + body.length, insert: "" }); }
...
export function formulaRuleEdits(text: string, separatorMinLength = 5, commands: string[] = [], enabledRuleIds: string[] = []): FormulaRuleEdit[] {
  const enabled = new Set(enabledRuleIds.length ? enabledRuleIds : ["formula-trim", "formula-double-escape", "formula-block-separator", "formula-code-wrapper", "formula-block-normalize"]);
  ...
  if (enabled.has("formula-restore-missing-delimiter")) {
    const style = delimiterStyle(enabled); const parens = /(?<!\\)\(([^()\n]+)\)/g;
    while ((match = parens.exec(text))) { const from = match.index; const to = from + match[0].length; if (!blocked(from, to, protectedRanges) && looksLikeMath(match[1])) add(result, "formula-restore-missing-delimiter", from, to, { from, to, insert: style === "dollar" ? `$${match[1]}$` : `\\(${match[1]}\\)` }); }
    const brackets = /^\[\n([\s\S]*?)\n\]$/gm;
    while ((match = brackets.exec(text))) { const from = match.index; const to = from + match[0].length; if (!blocked(from, to, protectedRanges) && looksLikeMath(match[1])) add(result, "formula-restore-missing-delimiter", from, to, { from, to, insert: style === "dollar" ? `$$${match[1]}$$` : `\\[${match[1]}\\]` }); }
  }
